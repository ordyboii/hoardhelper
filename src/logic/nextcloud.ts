...
export async function testConnection(url?: string, username?: string, password?: string): Promise<boolean> {
    ...
    if (!clientToTest) throw new Error("Client not initialized and no credentials provided");
...
async function ensureRemoteDir(remotePath: string): Promise<void> {
    ...
    const parts = remotePath.replace(/\\/g, '/').split('/').filter(p => p);
...
export async function uploadFileToNextcloud(
    ...
    if (!client) throw new Error("Client not initialized");
    if (!remoteDestination) return { success: false, error: "Invalid remote path" };
    ...
    console.log(`[Nextcloud] Starting upload (Stream): ${localPath} -> ${remoteDestination}`);
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
             ...
             localStream.on('error', (err) => {
                 ...
                 reject({ success: false, error: "Local file read error: " + err.message });
             });
