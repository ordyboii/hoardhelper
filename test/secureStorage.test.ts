...
describe("Secure Storage", () => {
  describe("Encryption fallback behavior", () => {
    it("should handle base64 encoding when encryption is unavailable", () => {
      ...
      assert.strictEqual(
        decoded,
        plaintext,
        "Base64 encode/decode should work",
      );
    });
  ...
  describe("StoredSettings structure", () => {
    it("should separate encrypted and plain fields", () => {
      ...
      // Verify structure
      assert.ok(
        storedSettings.password_encrypted,
        "Password should be encrypted",
      );
      assert.ok(
        storedSettings.realDebridApiKey_encrypted,
      ...
      assert.strictEqual(
        ...
        "user",
        "Username should be plain",
      );
      assert.strictEqual(
        storedSettings._encrypted,
        true,
        "Should have encryption flag",
      );
    });
    ...
    it("should not contain plain-text sensitive fields", () => {
      ...
      assert.strictEqual(
        ...
        undefined,
        "Should not have plain password field",
      );
      assert.strictEqual(
        storedSettings.realDebridApiKey,
        undefined,
        "Should not have plain API key field",
      );
    });
  });
  ...
  describe("Migration from legacy format", () => {
    it("should handle old unencrypted settings", () => {
      ...
      assert.strictEqual(
        legacySettings._encrypted,
        undefined,
        "Legacy format has no encryption flag",
      );
